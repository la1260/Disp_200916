...
//			String.format("%ty-%1$tm-%1$td %1$tH:%1$tM:%1$tS", nextms)+"\t"
	...
	public Main() {
		...
		rail.put(18, Rails.Type.R, 500, 27, 6, -1, -1, 90/3.6, -1, 4, 4, 12, 0, "I");
		rail.put(19, Rails.Type.R, 500, 28, 7, -1, -1, 90/3.6, -1, 4, 6, 12, 0, "II");
		rail.put(20, Rails.Type.R, 500, 12, 22, -1, -1, 90/3.6, -1, 22, 4, 12, 0, "III");
		rail.put(21, Rails.Type.R, 500, 13, 23, -1, -1, 90/3.6, -1, 22, 6, 12, 0, "IV");
		rail.put(22, Rails.Type.R, 500, 20, 29, -1, -1, 90/3.6, -1, 34, 4, 12, 0, "V");
		rail.put(23, Rails.Type.R, 500, 21, 30, -1, -1, 90/3.6, -1, 34, 6, 12, 0, "VI");
		rail.put(24, Rails.Type.R, 500, 14, 15, -1, -1, 80/3.6, -1, 22, 8, 12, 0, "VII");
		rail.put(25, Rails.Type.R, 500, 17, 26, -1, -1, 90/3.6, -1, 22, 14, 12, 0, "VIII");
		rail.put(26, Rails.Type.R, 500, 25, 31, -1, -1, 90/3.6, -1, 10, 14, 12, 0, "IX");
		...
		if (gitems.containsKey(rail_id)) {
			...
			gitems.put(rail_id, p= new JPanel() {
				...
				public void paint(Graphics g) {
					super.paint(g);
					String s;
					int train_id= rail.getUser(rail_id);
					if (train_id==-1) {
						s= rail.getName(rail_id);
					} else {
						s= String.valueOf(train_id);
					}
					switch (rail.getStatus(rail_id)) {
			...
			p.setBounds(rail.getX(rail_id)*8, rail.getY(rail_id)*16, (rail.getWidth(rail_id)+1)*8, (rail.getHeight(rail_id)+1)*16);
			p.setVisible(true);
		}
	}
	public void trainPassIn(int train_id, double t) {
		System.out.println("Behaladt: "+train_id+" t: "+Math.rint(t*10)/10);
	}
